...
export interface Product { id: ProductId; rank: number; name: string; tagline: string; description: string; buyer: string; price: string; accent: string; nav: string[]; metrics: { label: string; value: string }[]; records: { title: string; detail: string; status: ReviewStatus | EvidenceStatus; meta: string }[]; api: string[]; }
